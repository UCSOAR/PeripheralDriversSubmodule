...
bool KSZ9893RDriver::SPI_Read(uint16_t addr, uint8_t* data, uint16_t len) {
    ...
    uint8_t header[3];
    header[0] = KSZ_SPI_READ | KSZ_SPI_AI | ((addr >> 10) & 0x3F);
...
bool KSZ9893RDriver::SPI_Write(uint16_t addr, const uint8_t* data, uint16_t len) {
    ...
    uint8_t header[3];
    header[0] = KSZ_SPI_WRITE | KSZ_SPI_AI | ((addr >> 10) & 0x3F);
    header[1] = (addr >> 2) & 0xFF;
    header[2] = (addr & 0x03) << 6;
...
bool KSZ9893RDriver::SoftReset() {
    uint8_t val = ReadReg(KSZ9893R_REG::SWITCH_OP);
    WriteReg(KSZ9893R_REG::SWITCH_OP, val | 0x01);
...
bool KSZ9893RDriver::ConfigureRMII_Port3() {
    ...
    WriteReg(KSZ9893R_REG::PORT3_CLK_CTRL,  0x01); 
    WriteReg(KSZ9893R_REG::PORT3_CTRL_1,    0x01); 
    return true;
}
...
bool KSZ9893RDriver::Init(SPI_HandleTypeDef* hspi_, GPIO_TypeDef* cs_gpio_, uint16_t cs_pin_) {
    ...
    WriteReg(KSZ9893R_REG::PORT1_CTRL_1, 0x01);
    WriteReg(KSZ9893R_REG::PORT2_CTRL_1, 0x01);
    WriteReg(KSZ9893R_REG::PORT3_CTRL_1, 0x01);

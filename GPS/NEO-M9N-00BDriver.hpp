...
public:
	...
	bool Init(SPI_HandleTypeDef* hspi, GPIO_TypeDef* csPort, uint16_t csPin);
	bool getGGALine(char* lineBuf);
	void ParseGpsData(GPSData* data);

private:
	bool valSetLType(uint32_t keyID, uint8_t value);
	bool valSetU1Type(uint32_t keyID, uint8_t value);

	void computeUBXChecksum(const uint8_t* data, uint16_t len, uint8_t& ckA, uint8_t& ckB);

...
/**
 * @brief  Get an FDCAN frame from the Rx FIFO zone directly into the controller's back RX buf.
 * Adapted from the HAL_GetRxMessage function
 * @param  pRxHeader pointer to an output FDCAN_RxHeaderTypeDef structure.
 * @retval HAL status
 */
HAL_StatusTypeDef FDCanController::GetRxMessageDirect(
		FDCAN_RxHeaderTypeDef *pRxHeader)
{
	...
	if (state == HAL_FDCAN_STATE_BUSY)
	{
		...
		{
			/* Update error code */
			fdcan->ErrorCode |= HAL_FDCAN_ERROR_FIFO_EMPTY;
		...
		else
		{
			...
			if (fdcan->Init.RxFifo0ElmtsNbr > 0U) {
				GetIndex = GetIndex % fdcan->Init.RxFifo0ElmtsNbr;
			}
		...
		// Force 32-bit word accesses from SRAMCAN to avoid bus faults
		uint32_t *pDstWord = (uint32_t *)pRxData;
		for (uint32_t i = 0; i < wordsToRead; i++) {
			pDstWord[i] = pDataWord[i];
		}
...
#ifdef FDCAN_DEBUG
	memset(idReceiveTracker,0,sizeof(idReceiveTracker));
#endif
...
HAL_StatusTypeDef FDCanController::RegisterLogs(LogInitStruct *logs, uint16_t numLogs) {
	...
	if(numRegisteredLogs+numLogs > MAX_FDCAN_LOGS) {
		return HAL_ERROR;
	}
...
bool FDCanController::RemoveLogIndices(const uint8_t* indices, uint8_t count) {
	LogRegister registeredLogsMutable[MAX_FDCAN_LOGS];
	uint8_t c = 0;
	for(uint16_t i = 0; i < numRegisteredLogs; i++) {
		...
		if(!istoberemoved) {
			registeredLogsMutable[c++] = registeredLogs[i];
		}
	}
...
bool FDCanController::RebuildFilters() {
	...
	while(HAL_FDCAN_GetTxFifoFreeLevel(fdcan) < fdcan->Init.TxFifoQueueElmtsNbr && HAL_GetTick()-start < 500);
...
bool FDCanController::DiscardLog(uint16_t index) {
	...
	uint8_t msg[FDRoundDataSize(registeredLogs[index].byteLength)];
	return ReceiveLogIndexFromRXBuf(msg, index) > 0;
}

#ifdef FDCAN_DEBUG
void FDCanController::ReportIDsReceived() {
	bool foundOne = false;
...
uint32_t FDCanController::GetTicksSinceLastIDReceivePrint() {
	return HAL_GetTick() - lastReceivePrintTick;
}
